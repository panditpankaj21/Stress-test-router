...
def connect_wifi(context):
    ...
    code, out, err = run_cmd('sudo wpa_supplicant -B -i wlan0 -c /etc/wpa_supplicant/wpa_supplicant.conf')
...
def verify_single_network(context):
    ...
    assert f'ssid="{context.ssid}"' in config, f"SSID {context.ssid} not found in config"
