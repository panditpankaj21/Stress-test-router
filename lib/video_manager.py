...
class VideoManager:
    ...
    async def check_processes(self, ns):
        ...
        return {
            ...
            "yt-dlp_pids": (
                result_ytdlp["stdout"].strip()
                if result_ytdlp["returncode"] == 0
                else ""
            ),
            "mpv_pids": (
                result_mpv["stdout"].strip() if result_mpv["returncode"] == 0 else ""
            ),
        }
    ...
    async def verify_streaming(self, ns):
        ...
        # Overall verdict
        is_streaming = (
            process_check.get("yt-dlp_running", False)
            or process_check.get("mpv_running", False)
    ...
    def parse_streaming_info(self, stdout, stderr):
        ...
        for line in combined_output.split('\n'):
            ...
            if "A:" in line and "/" in line:
                ...
                match = re.search(
                    r'A:\s*(\d{2}:\d{2}:\d{2})\s*/\s*(\d{2}:\d{2}:\d{2})', line
                )
                if match:
    ...
    async def _stream_with_mpv(self, ns, video_id):
        ...
        if not is_success:
            logger.error(f"[ERROR] {ns} stream failed. Code: {result['returncode']}")
        ...
        return {
            ...
            "bandwidth_used": (
                final_verification["bandwidth"].get("bytes_transferred", 0)
                if final_verification["bandwidth"]
                else 0
            ),
    ...
    def display_results_table(self, results):
        ...
        for ns, result in results.items():
            ...
            table.add_row(
                [ns, status, duration, file_size, speed, resolution, audio, returncode]
            )
        ...
        logger.info("=" * 130)
        logger.info(
            f"Total Namespaces: {len(results)}"
            + f" | Success: {success_count} | Failed: {failed_count}"
            + f" | Success Rate: {(success_count/len(results)*100):.2f}%"
        )
        logger.info("=" * 130 + "\n")
